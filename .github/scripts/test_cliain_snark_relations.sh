...
# echo "Testing 'verdict' relation"
${CLIAIN} snark-relation generate-keys verdict
${CLIAIN} snark-relation generate-proof -p verdict.groth16.pk.bytes verdict \
    --max-votes-len 4 \
    --decoded-votes "1,1,1,0" \
    --hashed-votes "9935575782689025380,6968921470419806625,17618364339815705398,2126917312324872696" \
    --votes-sum 3 \
    --hashed-shared-keys "16840512679269690949,18008264538130655535,5145577776023086429,445979091408012580:8756224789056188938,16771720507772505288,6896133462370656363,716756380027199902:14593441714109358832,1880390010059943538,17090328113617597038,1984528836507263142:6746047069746938344,14870478950004271521,8513630371245202259,790900300225404089"
${CLIAIN} snark-relation verify \
    --verifying-key-file verdict.groth16.vk.bytes \
    --proof-file verdict.groth16.proof.bytes \
    --public-input-file verdict.groth16.public_input.bytes

# echo "Testing 'vote' relation"
# ${CLIAIN} snark-relation generate-keys vote
# ${CLIAIN} snark-relation generate-proof -p vote.groth16.pk.bytes vote \
#   --vote 1 \
#   --hashed-shared-key "2016731170274856920,6204751156390945474,5607385113875519496,7433880339709118400" \
#   --encrypted-vote "2016731183159758805,6549017004486250181,12980472491665756152,820073835025321959"
# ${CLIAIN} snark-relation verify \
#   --verifying-key-file vote.groth16.vk.bytes \
#   --proof-file vote.groth16.proof.bytes \
#   --public-input-file vote.groth16.public_input.bytes
